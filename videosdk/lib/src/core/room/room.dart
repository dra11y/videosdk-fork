...
class Room {
  ...
  Future<void> join() async {
    ...
    _webSocket!.onFail = () {
      try {
        _eventEmitter.emit('meeting-state-changed', RoomState.failed);
    ...
    _webSocket!.onNotification = (notification) async {
      switch (notification['method']) {
        ...
        case 'userMessage':
          {
            _eventEmitter.emit(
                "USER_MESSAGE", UserMessage.fromJson(notification['data']));
...
//
  ...
  void _close([String? errorMsg]) {
    ...
    try {
      _eventEmitter.emit('meeting-state-changed', RoomState.closing);
    ...
    _eventEmitter.emit('meeting-state-changed', RoomState.closed);
